...
In practice, transactions are sent to Execution Layer RPC nodes (e.g. via proxyd) and only transactions accepted into the EL txpool are propagated to the sequencer for potential inclusion.

Implementing mechanisms such as mempool rebroadcasting can mitigate this divergence automatically, so users do not notice retries. However, when an error is surfaced, it may originate from the ingress client’s gas checks.
