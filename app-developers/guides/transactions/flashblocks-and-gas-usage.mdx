...
A transaction may be submitted with a gas limit lower than the total block gas limit (e.g. **38M < 40M**) and still remain pending or be rejected with the error: `exceeds block gas limit`

This happens because transaction acceptance and transaction inclusion are decided by different components:

- Incoming transactions first enter the Execution Layer txpool (e.g. **op-geth**).
...
- A transaction must be **< the full block gas limit** to be accepted into the EL txpool, but it must fit within the **remaining block gas** to be included by rbuilder in the next flashblock 
