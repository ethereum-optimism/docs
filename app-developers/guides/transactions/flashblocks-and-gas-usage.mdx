...
In practice, transactions are first accepted by an RPC node and only later evaluated by the block builder (rbuilder). Because this process is asynchronous, a transaction may initially appear accepted but later be rejected by the builder due to remaining-gas constraints or timing differences.
