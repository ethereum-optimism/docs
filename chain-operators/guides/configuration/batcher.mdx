...
#### (EXPERIMENTAL) PID controller flags
These flags are only relevant when `--throttle.controller-type=pid`. The PID controller is experimental — adjust carefully.

##### throttle.pid-kp
PID proportional gain. Must be >= 0.

<Tabs>
  <Tab title="Syntax">`--throttle.pid-kp=<float>`</Tab>
  <Tab title="Example">`--throttle.pid-kp=0.33`</Tab>
  <Tab title="Environment Variable">`THROTTLE_PID_KP=0.33`</Tab>
</Tabs>

##### throttle.pid-ki
PID integral gain. Must be >= 0.

<Tabs>
  <Tab title="Syntax">`--throttle.pid-ki=<float>`</Tab>
  <Tab title="Example">`--throttle.pid-ki=0.01`</Tab>
  <Tab title="Environment Variable">`THROTTLE_PID_KI=0.01`</Tab>
</Tabs>

##### throttle.pid-kd
PID derivative gain. Must be >= 0.

<Tabs>
  <Tab title="Syntax">`--throttle.pid-kd=<float>`</Tab>
  <Tab title="Example">`--throttle.pid-kd=0.05`</Tab>
  <Tab title="Environment Variable">`THROTTLE_PID_KD=0.05`</Tab>
</Tabs>

##### throttle.pid-integral-max
PID maximum integral windup. Must be > 0.

<Tabs>
  <Tab title="Syntax">`--throttle.pid-integral-max=<float>`</Tab>
  <Tab title="Example">`--throttle.pid-integral-max=1000.0`</Tab>
  <Tab title="Environment Variable">`THROTTLE_PID_INTEGRAL_MAX=1000.0`</Tab>
</Tabs>

##### throttle.pid-output-max
PID maximum output (normalized between 0 and 1). Must be > 0 and <= 1.0.

<Tabs>
  <Tab title="Syntax">`--throttle.pid-output-max=<float>`</Tab>
  <Tab title="Example">`--throttle.pid-output-max=1.0`</Tab>
  <Tab title="Environment Variable">`THROTTLE_PID_OUTPUT_MAX=1.0`</Tab>
</Tabs>

##### throttle.pid-sample-time
PID sample time interval.

<Tabs>
  <Tab title="Syntax">`--throttle.pid-sample-time=<duration>`</Tab>
  <Tab title="Example">`--throttle.pid-sample-time=2s`</Tab>
  <Tab title="Environment Variable">`THROTTLE_PID_SAMPLE_TIME=2s`</Tab>
</Tabs>
