...
The batcher is configured for throttling by default with the parameters described below (and corresponding flags which allow the defaults to be overriden): 
...
*  Throttler controller type via `--throttle.controller-type` (env var `OP_BATCHER_THROTTLE_CONTROLLER_TYPE`): `quadratic` by default. This determines how transaction and block size limits are interoplated when throttling is at intermediate inensity.
