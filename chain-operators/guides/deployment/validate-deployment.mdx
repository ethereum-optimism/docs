...
*   Complete deployment of L1 smart contracts using [op-deployer](/chain-operators/deploy/smart-contracts).
*   Generate [genesis and rollup](/chain-operators/deploy/genesis) files.
*   Install [op-validator](/chain-operators/tools/op-validator).

## Validation process

<Steps>
  <Step title="Install op-validator">
  If you haven't already installed op-validator, follow the [installation instructions](/chain-operators/tools/op-validator#installation).
  </Step>
  ...
  <Step title="Run validation for your contract version">
  Run the op-validator with the appropriate version parameter matching your deployed contracts:

    ```bash
      ./bin/op-validator validate <version of op-contracts i.e. v2.0.0> \
  ...
  <Step title="Address any validation errors">
...
*   If you plan to join the Superchain, follow the [Standard Rollup Charter](/superchain/concepts/blockspace-charter).
