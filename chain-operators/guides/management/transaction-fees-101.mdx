...
- `eip1559Denominator`: EIP-1559 max-change denominator, it’s in the denominator of the per-block base fee delta. Lower = faster response, so base fee adjust faster but results in more volatility.
- `eip1559Elasticity`: elasticity multiplier, sets the target relative to gasLimit. As `gas_target = gasLimit / elasticity`, higher elasticity results in smaller target and base fee will increase sooner when blocks are busy.
- `gasLimit`: max gas per block. Lower = more scarcity and therefore how easily the base fee moves.
...
- **Cons:** UX impact, as it might hit low-value transactions. Changes transaction economics and should be used carefully. Any non-zero operator fee makes the chain configuration non-standard.
