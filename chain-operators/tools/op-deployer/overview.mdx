...
<Warning>
  The `upgrade` command in OP Deployer has been deprecated and is no longer maintained. Use the [OPCM upgrade interface](/chain-operators/reference/opcm) directly for chain upgrades. See the [deprecation notice](/notices/op-deployer-upgrade-deprecation) for details.
</Warning>
