---
title: OP Deployer
description: A CLI tool for deploying smart contracts for OP Stack chains.
---

OP Deployer is a CLI tool that simplifies deploying smart contracts for OP Stack chains. It also
exposes a suite of libraries that allow developers to easily manage smart contracts from their applications.

<Warning>
  The `upgrade` and `manage` commands in OP Deployer have been deprecated and are no longer maintained. Use the [OPCM upgrade interface](/chain-operators/reference/opcm) directly for chain upgrades and chain management. See the [deprecation notice](/notices/op-deployer-upgrade-deprecation) for details.
</Warning>
...
API are optimized for deploying Standard OP Chains. However, it also exposes a lower-level set of
