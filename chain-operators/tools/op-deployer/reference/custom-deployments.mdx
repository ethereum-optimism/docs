...
<Warning>
The `upgrade` and `manage` commands in `op-deployer` have been deprecated and are no longer maintained. For chain upgrades, use the [OPCM upgrade interface](/chain-operators/reference/opcm) directly. See the [deprecation notice](/notices/op-deployer-upgrade-deprecation) for details.
</Warning>
