...
implementation, please refer to [MIPS reference](/stack/fault-proofs/mips).
...
The information stored is largely identical to the [VM execution state](/stack/fault-proofs/mips#packed-vm-execution-state) for `MIPS.sol`. The key differences are:
...
calldata can be seen in the [`MIPS.sol` documentation](/stack/fault-proofs/mips#contract-state).
...
certain syscalls. The full list of instructions supported can be found [here](/stack/fault-proofs/mips#table-of-supported-mips-instructions).
