...
Following the [Ecotone upgrade](/op-stack/protocol/network-upgrades#ecotone)
node operators must set an L1 beacon value to retrieve [blobs](/node-operators/guides/management/blobs)
from a Beacon node.
...
## Next steps
*   See the [`op-node` configuration](/node-operators/guides/configuration/consensus-config) guide for additional configuration options for `op-node` and the Consensus-Layer.
*   For execution client configuration, visit [execution client configuration](/node-operators/guides/execution-config) for additional options when using `op-geth` or `nethermind`
