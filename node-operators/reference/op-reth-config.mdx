...
The following options are from [v1.9.3](https://github.com/paradigmxyz/reth/releases/tag/v1.9.3)
...
<Tabs>
  ...
  <Tab title="Default">`default` (OS-specific)</Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Default">`reth/v1.9.3...`</Tab>
</Tabs>
...
<Tabs>
  <Tab title="Syntax">`--net-if.experimental`</Tab>
...
Disables gossiping of transactions in the mempool to peers. 
This can be omitted for personal nodes, though providers should always opt to enable this flag.
...
Always process payload attributes and begin a payload build process even if forkchoiceState.headBlockHash is already the canonical head.

Note: This is a no-op on the OP Stack.
...
Allow unwinding the canonical header to an ancestor during forkchoice updates.
...
<Tabs>
  ...
  <Tab title="Default">`2x available parallelism (clamped 2-64)`</Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Default">`Same as storage-worker-count`</Tab>
</Tabs>
