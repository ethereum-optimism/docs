...
<Info>
This fork inherits all standard op-reth configuration options. [op-reth configuration reference](/node-operators/reference/op-reth-config).
</Info>
...
```bash
op-reth proofs init --datadir <DATA_DIR> --proofs-history.storage-path <PROOFS_HISTORY_STORAGE_PATH> 
```
...
```bash
op-reth proofs prune --datadir <DATA_DIR> --proofs-history.storage-path <PROOFS_HISTORY_STORAGE_PATH> --proofs-history.window <PROOFS_HISTORY_WINDOW>
```
