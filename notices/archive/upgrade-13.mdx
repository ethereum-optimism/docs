...
*   **OP Contracts Manager**: A new system for upgrading L1 contracts across OP Stack chains, making contract upgrades more scalable, less prone to error, and easier to test.
...
*   **`DeputyPauseModule` (OP Stack pause improvements)**: A new Safe Module to be installed into the Optimism Foundation Safe to simplify the process of quickly responding to security incidents via the OP Stack-wide pause mechanism.
...
The OP Contracts Manager ([OPCM](/chain-operators/reference/opcm)) is a new system for upgrading L1 contracts across OP Stack chains. Each release will have its own OPCM that can deploy new proxies and upgrade existing OP Chains.
...
### DeputyPauseModule (OP Stack pause improvements)
...
*   The Pause Deputy to create signatures that authorize the use of the OP Stack-wide pause
