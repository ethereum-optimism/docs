...
<Info>
  The Isthmus hard fork for the Sepolia OP Stack chains will be activated at **Thu Apr 17 16:00:00 UTC 2025** (`1744905600`) and the Mainnet OP Stack chains will be activated at **Fri May 9 2025 16:00:01 UTC** (`1746806401`).
  This is for all chains who have opted into the [hard fork inheritance behavior](/op-stack/ecosystem/superchain-registry#hard-fork-activation-inheritance-behavior).
</Info>
...
*   **L2 Withdrawals Root in Block Header**: This lowers the lift for chain operators by allowing them to run a full node to operate `op-dispute-mon` making it easier to guarantee the security of the fault proofs for OP Stack chains as the number of chains scales. Learn more about this [here](https://gov.optimism.io/t/proposal-preview-l2-withdrawals-root-in-block-header/9730).
...
Chains running permissionless fault proofs will need to deploy new dispute game contracts with new absolute prestates. The new 64 bit version of cannon will be utilized moving forward. The Sepolia OP Stack chains will utilize [op-program/v1.6.0-2](https://github.com/ethereum-optimism/optimism/tree/op-program/v1.6.0-rc.2). The following permissionless fault proof Sepolia chains are: `Base Sepolia`, `Ink Sepolia`, and `OP Sepolia`.
...
<Steps>
  ...
  The releases contain both the Isthmus Mainnet and Sepolia OP Stack chain activation timestamps.
