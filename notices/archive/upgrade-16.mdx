...
This page outlines important changes related to Upgrade 16 for chain operators and users. The upgrade proposal includes modifications to the OP Stack for continued success through the remainder of 2025.

<Info>
  The Upgrade 16 protocol upgrade on the **Sepolia** OP Stack chains will be executed on **Wed, Jul 09, 2025**, and the **Mainnet** OP Stack chains will be activated on **Thu, Jul 24, 2025 at 5:30 PM UTC (`1753675800`)**.\
...
*   **Interop-Ready smart contracts**: Interoperability is critical to realizing a unified network of OP Stack chains. This upgrade begins the rollout of foundational interoperability features by updating the `OptimismPortal` to handle future cross-chain messaging safely and extensibly. This upgrade does not turn on interop yet.
...
*   The pause action can now be applied on a per-chain basis as well as an OP Stack-wide basis
