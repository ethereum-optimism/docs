...
<Info>
  The `cannon`,`op-program`, `op-program host`, `kona-client`, and `kona-host` executables are run in the `op-challenger` docker container as sub-processes when required to generate game trace data.
</Info>
