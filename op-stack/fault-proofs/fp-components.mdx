...
The Fault Proof (FPP) is one of the modules in the OP Stack's fault proof system. 
It is a combination of both the consensus and execution "parts" of the protocol in a single process. This means Engine API calls that would normally be made over HTTP are instead made as direct method calls to the exection client code.
