...
import { Steps, Step } from "/snippets/steps.jsx"
...
<Steps>
  <Step title="Verify the chain configuration directory exists">
  ...
  <Step title="Copy your chain configuration files to this directory">
  ...
  <Step title="Generate the prestate">
  After placing your configuration files in the proper location, run the following command from the root of the monorepo:
  ...
  <Step title="Locate and prepare the preimage File">
...
<Steps>
  <Step title="Option 1: Using HTTP URL (Recommended for production)">
