...
<Warning>This feature is currently in development.</Warning>
<Info>
...
```bash
  ...
  <Step title="Enter the Optimism Monorepo">
```bash
  cd optimism
  ```
...
```bash
  ...
  <Step title="Build the various packages inside of the Optimism Monorepo">
```bash
...
```bash
  ...
  <Step title="Duplicate the sample environment variable file">
```bash
  ...
  <Step title="Fill out the environment variable file">
...
```bash
  ...
  <Step title="Generate new addresses">
<Info>
...
```bash
  ...
  <Step title="Load the variables with direnv">
<Info>
  ...
  **You *must* rerun the following command every time you change the `.envrc` file.**
  ...
  <Step title="Confirm that the variables were loaded">
...
```bash
  ...
  <Step title="Move into the contracts-bedrock package">
```bash
...
```bash
  ...
  <Step title="Generate the configuration file">
Run the following script to generate the `getting-started.json` configuration file inside of the `deploy-config` directory.
  ...
  <Step title="Review the configuration file (Optional)">
...
<Steps>
  <Step title="Check if the factory exists">
The Create2 factory contract will be deployed at the address `0x4e59b44847b379578588920cA78FbF26c0B4956C`.
  ...
  <Step title="Fund the factory deployer">
...
Using `cast`, deploy the factory contract to your L1 chain:
  ...
  <Step title="Wait for the transaction to be mined">
Make sure that the transaction is included in a block on your L1 chain before continuing.

  
  </Step>
  
  <Step title="Verify that the factory was deployed">
...
<Steps>
  <Step title="Navigate to the op-geth directory">
```bash
  cd ~/op-geth
  ```
  ...
  <Step title="Create a data directory folder">
```bash
  mkdir datadir
  ```
  ...
  <Step title="Build the op-geth binary">
```bash
  make geth
  ```
...
<Steps>
  <Step title="Open up a new terminal">
...
You'll need a terminal window to run the `op-batcher` in.
  ...
  <Step title="Navigate to the op-batcher directory">
```bash
...
You'll need a terminal window to run the `op-proposer` in.
  ...
  <Step title="Navigate to the op-proposer directory">
```bash
...
<Steps>
  <Step title="Navigate to the contracts-bedrock directory">
```bash
  ...
  <Step title="Get the address of the L1StandardBridgeProxy contract">
```bash
  ...
  <Step title="Send some Sepolia ETH to the L1StandardBridgeProxy contract">
