...
<Steps>
  ...
  <Steps>
    {<h4>Create a folder and new file</h4>}

    ```bash
    mkdir op-sample-project
    cd op-sample-project
    touch estimate-costs.js
    ```
...
<Steps>
  ...
  In your project directory and add the following code in `estimate-costs.js`:
...
<Callout type="info">
  To ensure that `process.env.PRIVATE_KEY` is defined before using it. If they are undefined, we add a check to provide a error messages when the environment variable is missing.
</Callout>
