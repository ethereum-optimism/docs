...
This guide teaches you how to add OP Mainnet to your Centralized Exchange. It reviews how to connect to OP Mainnet, make deposits and withdrawals in OP Mainnet, and access audit reports.
...
You can choose between the Optimism public endpoints, which are rate limited, and endpoints from [infrastructure providers](/chain/networks). Given rate throughput limits, it is recommended to use a private RPC provider for both mainnet and testnet use cases.
...
For example, looking at the **SNX** token, you get these addresses:
...
Most of the cost of an OP Mainnet transaction is not the gas consumed by the transaction itself (which is rarely above 0.001 gwei per gas), but the cost of writing the transaction in Ethereum. That cost is deducted automatically from the user's address. If you charge your users the cost of withdrawals, you have to account for it.

You can read more about [transaction fees](/stack/transactions/transaction-fees).
The relevant code is [here](/builders/dapp-developers/tutorials/sdk-estimate-costs).

## Deposits and withdrawals across chains

As a centralized exchange, there will be times that withdrawals of ETH or an ERC-20 token on either OP Mainnet or Ethereum exceed deposits and you need to transfer ETH or tokens.
To do that you use a bridge or a gateway.
OP Mainnet has a [standard gateway](https://app.optimism.io/bridge) that receives tokens on L1 (Ethereum mainnet), and mints the equivalent tokens on OP Mainnet.
When a user wants to withdraw the tokens back to L1, the bridge burns the token on L2 and releases it to the user on L1. If you want to use this gateway automatically, [follow this tutorial for ETH](/builders/dapp-developers/tutorials/cross-dom-bridge-eth) or [this one for ERC-20 tokens](/builders/dapp-developers/tutorials/cross-dom-bridge-erc20).

Note that while L1 to L2 transactions typically take minutes, L2 to L1 transactions on the gateway require [a seven day challenge period](https://help.optimism.io/hc/en-us/articles/4411895558171-Why-do-I-need-to-wait-a-week-when-moving-assets-out-of-Optimism-).

Alternatively, you can use a [third party bridge](https://www.optimism.io/apps/bridges). These bridges usually rely on liquidity pools to allow for faster withdrawals and support multiple L2 chains. However, their token selection might be more limited and they may not be as decentralized as the Optimism Gateway.
