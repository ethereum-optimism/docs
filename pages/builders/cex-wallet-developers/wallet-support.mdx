...
You'll find all of the important information about OP Mainnet, as well as any test networks, on [our Networks page](/chain/networks).
You can choose to connect to OP Mainnet through our rate-limited public endpoints, [private endpoints from infrastructure providers](/chain/networks), or [by running your own node](/builders/node-operators/tutorials/node-from-source).
...
We're still working on the tools needed to easily detect when a given transaction has been published to Ethereum.
For now, we recommend wallets consider transactions final after they are "Sequencer Confirmed".
