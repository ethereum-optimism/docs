...
perspective. The OP Stack is built to be as similar to
...
`op-batcher` submits batch data to L1, which controls the safe blocks and ultimately
controls the canonical chain, while `op-proposer` submits output roots to L1 which control
L2 to L1 messaging.

<Image src="/img/builders/chain-operators/sequencer-diagram-background.png" alt="Sequencer Component Diagram" width={400} height={400} />
...
`op-challenger` is the service that monitors output roots submitted to L1 and disputes invalid claims. 
It ensures the integrity of L2 state transitions by challenging incorrect output roots, reinforcing trustless security.

## Sequencer

The Sequencer node works with the batcher and proposer to create new blocks,
handling state-changing RPC requests like `eth_sendRawTransaction`. 
It can peer with replica nodes to gossip new unsafe blocks across the network.

For security and stability, Sequencer op-node should have P2P discovery disabled and be statically peered with internal nodes.
Alternatively, it can use peer-management-service to define its peering network.

A production setup should include:

*   At least one archive sequencer for recovering from deep L1 reorgs
*   A secure P2P configuration with static peering
*   Integration with op-conductor for high availability
*   P2P discovery disabled with a defined static peer list for internal nodes.
...
<Image src="/img/builders/chain-operators/replica-node-background.png" alt="Replica Node Diagram" width={200} height={200} />
...
It is important to set up a robust chain architecture to handle large volumes of RPC
