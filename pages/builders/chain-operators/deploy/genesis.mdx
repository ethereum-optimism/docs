...
We do not recommend using this method. It's preserved here for historical context.
</Callout>
...
2. **Generate configuration files**  
   Use the `op-deployer` commands to create the required files. 
   With the [contracts deployed](builders/chain-operators/deploy/smart-contracts#using-op-deployer), generate the genesis and rollup configuration files by running the following commands:
