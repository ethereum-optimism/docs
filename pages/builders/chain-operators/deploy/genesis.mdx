...
Create or modify a file `<network-name>.json` inside of the `deploy-config`
...
variable `OUTPUT_MODE` allows to modify this behavior by setting it to one of
...
*   `DEPLOY_CONFIG_PATH` represents to a path on the filesystem that points to a
    deploy config. The same deploy config JSON file should be used for L1 contracts
