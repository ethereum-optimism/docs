...
This release was cut before the `L2Genesis.s.sol` script was added. Instead of
a solidity script, you can use the `op-node` to create your genesis file.
Within the `op-node` directory of the monorepo, you can 

```bash
go run cmd/main.go genesis l2 \
  --deploy-config <<Path to deploy config file>> \
  todo: get clarity on this flag (ath to network deployment directory. Cannot be used with --l1-deployments)
  --deployment-dir ../packages/contracts-bedrock/deployments/getting-started/ \
  todo: shoudl we use this instead?
