...
In this case, this [top level query](https://docs.blockscout.com/for-users/api/graphql#queries) is for multiple addresses.

Note that you can only query on fields that are indexed.
For example, the following is a query on the addresses.
However, you couldn't query on `contractCode` or `fetchedCoinBalance`.
...
The API does not let us fetch an unlimited number of transactions, so here the query asks for the first 5.
...
These are the fields read for each transaction.
