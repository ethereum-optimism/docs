...
<Steps>
  ...
  *   Configure challenger with the required flags. Tip: Use the `op-challenger --help` to view all of the subcommands, command line and environment variable options.
  ...
  *   Note: Challenger has a lot of money and it will spend it if it needs to interact with games. That might risk not defending games or challenging games correctly, so chain operators should really trust the nodes being pointing at Challenger.
  ...
  *   perform moves in games and then claim and resolve things.
  ...
  Chain operators can easily create their grafana dashboard for Dispute Monitor using the following json file: [Download](/grafana/dispute-monitor-1718214549035.json).
</Steps>
