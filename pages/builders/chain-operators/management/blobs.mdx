...
<Callout type="warning">
  The scalar values you use on your chain are coupled with the data throughput 
  of your chain. You can utilize this [fee parameter calculator](https://docs.google.com/spreadsheets/d/12VIiXHaVECG2RUunDSVJpn67IQp9NHFJqUsma2PndpE/edit)
  to get a better estimate for scalars values on your chain.
</Callout>
...
**The values in the following steps have example values.** You should utilize this
[fee parameter calculator](https://docs.google.com/spreadsheets/d/12VIiXHaVECG2RUunDSVJpn67IQp9NHFJqUsma2PndpE/edit)
to get a better estimate for scalar values on your chain.
...
<Steps>
  ...
  If your chain is submitting full blobs, rough *example* scalar values to set 
  to target a minimal margin on costs are:

  ```
  l1BaseFeeScalar = 1100
  l1BlobBaseFeeScalar = 655000
  ```
  ...
  Assuming a 5 hour `OP_BATCHER_MAX_CHANNEL_DURATION`, this is how to configure 
  on the `op-batcher`. If you are posting 65KB per 5 hours of data (assuming 
  roughly 250 byte transactions, this would be \~1250 transactions per day), 
  then you would want to double the above example scalar values:

  ```
  baseFeeScalar = 2200 = 1100 * 2
  blobBaseFeeScalar = 1310000 = 655000 * 2
  ```
...
back to posting data to calldata. Chains can follow these steps to switch from 
