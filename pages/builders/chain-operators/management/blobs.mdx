...
<Steps>
  ...
  <Callout type="warning">
    However, if your chain has less data throughput than 120kb per batch submission, you may have to only partially fill blobs.
    Because you must purchase an entire blob, you will be paying for 120KB of data, but submitting less, and must adjust your scalar accordingly.
  </Callout>

  Assuming a 5 hour `OP_BATCHER_MAX_CHANNEL_DURATION`, this is how to configure on the `op-batcher`.
  If you are posting 60KB per 5 hours of data (assuming roughly 250 byte transactions, this would be \~1250 transactions per day), then you would want to double the above recommended scalar values:

  ```
  baseFeeScalar = 15200 = 7600 * 2
  ...
  *   Ensure your `OP_BATCHER_MAX_CHANNEL_DURATION` is properly set to maximize your fee savings. **TODO: see "Chain Operator Configuration" page**
</Steps>
