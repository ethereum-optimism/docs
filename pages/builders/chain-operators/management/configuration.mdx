...
| Key                                 | Type   | Description                                                                                               | Default value |
| ----------------------------------- | ------ | --------------------------------------------------------------------------------------------------------- | ------------- |
| `OP_BATCHER_MAX_CHANNEL_DURATION`   | Number | The max time (in L1 blocks, which are 12 seconds each) between which batches will be submitted to the L1. | 1500          |
| `OP_BATCHER_BATCH_TYPE`             | Number | The number span batches, optional                                                                         | 1             |
| `OP_BATCHER_DATA_AVAILABILITY_TYPE` | String | Set batcher type is a string that can be set to 'calldata' or 'blobs'                                     | X             |
| `OP_BATCHER_TARGET_NUM_FRAMES`      | Number | The number of blobs per transaction                                                                       | X             |
| `OP_BATCHER_TXMGR_MIN_BASEFEE`      | Number | The amount of gwei, might need to tweak, depending on gas market                                          | X             |
| `OP_BATCHER_TXMGR_MIN_TIP_CAP`      | Number | The amount of gwei, might need to tweak, depending on gas market                                          | X             |
| `OP_BATCHER_RESUBMISSION_TIMEOUT`   | Number | Amount of time to wait before bumping fees                                                                | X             |
...
#### Configure Your Multi-Blob Batcher
