...
#### Setting your `Ol2OutputOracleSubmissionInterval`

Chain operators can post batches as infrequently as every 10 hours (within the 12 hour sequencer window) to save on costs. Recommendations for chain operators:

*   A maximum batch submission interval of \~6 hours to guarantee that you get your batches get in on time and you do not trigger reorgs.
*   As your chain grows in usage, and apps like CEXes and bridges start relying on the safe head for things like deposits and transfers, chain operators should start to drop the batch submission interval.
...
For more information, see the [Ecotone Upgrade guide](../ecotone) for chain operators.
...
<details>
  <summary><h4>Setting your  `OP_BATCHER_MAX_CHANNEL_DURATION`</h4></summary>
