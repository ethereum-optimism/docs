...
Snap sync works by downloading a snapshot of the state from other nodes on the network, allowing it to start executing blocks from the completed state rather than having to re-execute every single block.
