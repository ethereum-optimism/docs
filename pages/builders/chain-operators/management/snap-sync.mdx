...
<Callout type="warning">
  For snap sync, all `op-geth` nodes should be ran with the [`--discovery.port=30303`](/builders/node-operators/management/configuration#discoveryport) exposed to easily find other op-geth nodes to sync from.
...
<Steps>
  ...
  *   Expose op-geth [discovery port](/builders/node-operators/management/configuration#discoveryport) to the internet and figure out the default port and if there's any conflict with `op-geth`
