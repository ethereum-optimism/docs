...
4.  **Proposer**: The Proposer is a service responsible for publishing transactions *results* (in the form of L2 state roots) to the L1 blockchain. This allows smart contracts on L1 to read the state of the L2, which is necessary for cross-chain communication and user withdrawals.
...
| [Forcing Withdrawal from an OP Stack Blockchain](/stack/security/forced-withdrawal)    | Learn how to withdraw ETH or tokens on an OP Stack blockchain directly from L1.                                                        | 🟢 Easy          |
