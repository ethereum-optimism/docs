...
todo: turn into a table and clarify each status

*   `sequencer-0`: follower, active, paused, op-node conductor disabled
*   `sequencer-1`: follower, stopped, paused, op-node conductor disabled 
*   `sequencer-2`: leader, stopped, paused, op-node conductor disabled

{<h3>Switch the active sequencer</h3>}

*   Rollout `sequencer-2` as the active sequencer. You can use a blue/green 
style deployment to activate it.
...
todo: review PR1 in runbook is covered in this step
...
*   sequencer-0 -> follower, stopped, paused, op-node conductor enabled
*   sequencer-1 -> follower, stopped, paused, op-node conductor enabled
*   sequencer-2 -> leader, active, paused, op-node conductor enabled

{<h3>Resume conductor</h3>}

todo: link rpc method and clean up

rpc call to resume conductor on all nodes

{<h3>Confirm conductor has resumed</h3>}

todo: link rpc method and clean up

confirm all conductors successfully resumed

{<h3>Remove paused configuration</h3>}

todo: confirm PR2 is covered and clean up

Remove `OP_CONDUCTOR_PAUSED=true`.

{<h3>Set sequencer-0 to leader</h3>}

todo: confirm zdd workflow sequence
todo: add zdd workflow to a best practice document
...
todo: see if there are other run books we can document 
todo: get devinfra's review 
todo: make proposal, put runbooks in the public and change the codeowner in the docs to the eng owner
