...
To install from source, follow the steps below. You will need to install the Go toolchain and `just` as prerequisites.
...
`op-deployer` can also be used to deploy the contracts it needs to run the `init`... `apply` flow on new chains.
This process is called "bootstrapping," and is useful for when you want to use `op-deployer` with L3s, new testnets,
or other custom settlement chains.
