...
You should **NEVER** use the `develop` or `master` branches in production.
</Callout>
...
<Callout>
You will be using the `v1.101311.0` tag of the op- to deploy an OP Stack testnet chain during this tutorial.
You should **NEVER** use the `develop` or `master` branches in production.
</Callout>

*   `--branch v1.101311.0`: Specifies the `v1.3.0` smart contract release tag to checkout immediately after cloning.
...
todo: decide if we should switch to release v.1.7.4, its probably the move 
and then we can use the new solidity genesis generation process. See `Solidity L2 Genesis Generation: What is New` notion doc
