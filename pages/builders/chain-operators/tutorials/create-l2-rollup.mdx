...
You can either use a node provider (easier) or run your own Sepolia node (harder).
...
```bash
cd ~
git clone --branch op-contracts/v1.3.0 --recurse-submodules https://github.com/ethereum-optimism/optimism.git
```

<Callout>
You will be using the `op-contracts/v1.3.0` branch of the Optimism Monorepo to deploy an OP Stack testnet chain during this tutorial.
You should **NEVER** use the `develop`, `master`, or non-audited/governance approved branches in production.
</Callout>

*   `--branch op-contracts/v1.3.0`: Specifies the `v1.3.0` smart contract release tag to checkout immediately after cloning.
*   `--recurse-submodules`: Ensures that all submodules (and their submodules, recursively) are initialized and updated. This gets the necessary smart contract dependencies.
...
{<h3>Build smart contract package</h3>}
...
**If you don't see this output, you likely haven't [properly configured `direnv`](#direnv).**
...
Set up the configuration file for your chain.
...
Now that we have deployed the contracts on L1, we can turn our focus to Layer 2.

### Build the Optimism Monorepo

<Steps>

{<h3>Checkout Go Code Branch</h3>}
...
<Callout type="info">
Tags of the form `v<semver>`, such as `v1.7.4`, indicate releases of all Go 
code only, and **DO NOT** include smart contracts. This branch is for `op-node`,
...
{<h3>Build the client and services</h3>}
...
```bash
cd ~
git clone --branch v1.101311.0 https://github.com/ethereum-optimism/op-geth.git
```

<Callout type="info">
You will be using the `v1.101311.0` release of `op-geth` to deploy an OP Stack testnet chain during this tutorial.
Best practice is to use the latest release.
</Callout>

*   `--branch v1.101311.0`: Specifies the `v1.101311.0` release tag to checkout immediately after cloning.
...
todo: rewrite this section, see `Solidity L2 Genesis Generation: What is New` notion doc
...
{<h3>Navigate to the smart contract package</h3>}
