...
<Steps>
  ...
  | `privKey`     | The private key for an address that has some ETH on the L1                                  |
...
<Steps>
  ...
  ### Get ETH available after transfer
  ...
  ### See that the ETH available on L1 changed

  It probably changed by a lot more than 1 gwei because of the cost of the transaction.

  ```js
  (initialETH[0] - finalETH[0])/1e9
  ```

  ### See that the ETH available on L2 changed

  This process might take a few minutes.

  ```js
  ((await l2Provider.getBalance(l1Signer.address)) - initialETH[1])/1e9
  ```
</Steps>
