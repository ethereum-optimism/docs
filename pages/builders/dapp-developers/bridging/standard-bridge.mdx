...
lang: en-US
description: Learn the basics of using the standard bridge to move ERC-20 assets between Layer 1 and Layer 2.
---
...
Certain interactions, like transferring ETH and ERC-20 tokens between the two networks, are common enough that we've built the "Standard Bridge" to make moving these assets between L1 and L2 as easy as possible.
...
The Standard Bridge is composed of two main contracts the [`L1StandardBridge`](https://github.com/ethereum-optimism/optimism/blob/65ec61dde94ffa93342728d324fecf474d228e1f/packages/contracts-bedrock/contracts/L1/L1StandardBridge.sol) (for Layer 1) and the [`L2StandardBridge`](https://github.com/ethereum-optimism/optimism/blob/65ec61dde94ffa93342728d324fecf474d228e1f/packages/contracts-bedrock/contracts/L2/L2StandardBridge.sol) (for Layer 2). The contract addresses can be found [here](/chain/networks#contract-addresses).

Here we'll go over the basics of using this bridge to move ERC-20 assets between Layer 1 and Layer 2.

<Callout type="info">
  [See here for a step by step tutorial](../tutorials/cross-dom-bridge-erc20)
</Callout>
...
<Callout type="danger">
  *   Be sure to use the standard bridge contract only with standard bridge tokens.
...
Note that if you use the [OP Mainnet bridge UI](https://app.optimism.io/bridge), or the [Optimism SDK](https://sdk.optimism.io/) which automatically chooses the correct bridge contract, so this problem does not happen.
...
<Callout type="danger">
  USDC.e and USDC are different token contracts. If you're using `depositERC20` or `depositERC20To` make sure you're bridging to the correct destination address. Mixing up the mapping will result in your assets being stuck in the bridge.
</Callout>
...
If you'd like to see this contracts in action, you should check out the [L1 ⇔ L2 deposit-and-withdraw example](../tutorials/cross-dom-bridge-erc20).
...
<Callout type="info">
  To add your token to the standard bridge, see the guide [Adding an ERC20 token to the Standard Bridge](../tutorials/standard-bridge-standard-token).
</Callout>
...
If you're looking for an example to follow, take a look at [this simple pull request](https://github.com/ethereum-optimism/ethereum-optimism.github.io/pull/319/files) that adds a token to the token list.
