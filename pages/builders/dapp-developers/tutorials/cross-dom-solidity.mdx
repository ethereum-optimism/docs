...
Read more about message passing in the guide to [Sending Data Between L1 and L2](/builders/dapp-developers/bridging/messaging).
...
You're about to use two contracts that have already been deployed to Goerli and OP Goerli, the `Greeter` contracts.
You can review the source code for the L1 `Greeter` contract [here on Etherscan](https://goerli.etherscan.io/address/0xA1518896BA922cCD9fB83adb1fF549406FcF261C#code).
You can review the source code for the L2 `Greeter` contract [here on Etherscan](https://goerli-optimism.etherscan.io/address/0x31A6Dd971306bb72f2ffF771bF30b1B98dB8B2c5#code).
...
Open up the [L1 `Greeter` contract on Goerli Etherscan](https://goerli.etherscan.io/address/0xA1518896BA922cCD9fB83adb1fF549406FcF261C#writeContract) and click the "Connect to Web3" button.
...
Open up the [L2 `Greeter` contract on OP Goerli Etherscan](https://goerli-optimism.etherscan.io/address/0x31A6Dd971306bb72f2ffF771bF30b1B98dB8B2c5#readContract) and click the "Read Contract" button.
...
You'll do this by sending two transactions on Goerli, one proving transaction and one relaying transaction.
...
Open up the [L2 `Greeter` contract on OP Goerli Etherscan](https://goerli-optimism.etherscan.io/address/0x31A6Dd971306bb72f2ffF771bF30b1B98dB8B2c5#writeContract) and click the "Connect to Web3" button.
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L3 hash=26b2fdb17dd6c8326a54ec51f0769528
```

{<h3>Import ethers.js</h3>}

```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L4 hash=69a65ef97862612e4978b8563e6dbe3a
```

{<h3>Load your private key</h3>}

```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L6 hash=755b77a7ffc7dfdc186f36c37d3d847a
```

{<h3>Load your transaction hash</h3>}

```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L8 hash=320cd4f397d7bed8d914d4be0c99f8dc
```

{<h3>Create the RPC providers and wallets</h3>}

```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L10-L13 hash=4f453e218fe6079f35e4e12c5768acaa
```
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L15-L20 hash=3648c5f76a6ad1042083c54be8acd960
```
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L23 hash=25a072666b6147f8d8983d8223f045b8
```
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L26 hash=17922abea43b3d379404fedd87422dde
```
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L29 hash=45d995aab47ec29afee4bb4577ae9303
```
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L32 hash=b5515811ffcf8b9ada15dea8ae666e44
```
...
```js file=<rootDir>/public/tutorials/cross-dom-solidity.cjs#L35 hash=d6e7f89e929eea0ac3217a6751b7e578
```
...
Open up the [L1 `Greeter` contract on Goerli Etherscan](https://goerli.etherscan.io/address/0xA1518896BA922cCD9fB83adb1fF549406FcF261C#readContract) and click the "Read Contract" button.
Paste your address into the field next to the "greeting" function and click the "Query" button.
...
The final parameter the gas limit that gets used when the message is relayed on the other side.
...
You just learned how you can write Solidity contracts on Goerli and OP Goerli that can talk to each other.
...
For example, the [Standard Bridge](/builders/dapp-developers/bridging/standard-bridge) contracts use this same system to bridge ETH and ERC-20 tokens between Ethereum and OP Mainnet.
