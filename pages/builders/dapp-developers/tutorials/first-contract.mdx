...
## Add OP Goerli to MetaMask

This tutorial will show you how to deploy a smart contract to OP Goerli, the testnet for OP Mainnet.
Once you know how to deploy a smart contract to OP Goerli, you'll be able to deploy a smart contract to OP Mainnet in exactly the same way.
In order to interact with OP Goerli using MetaMask, you'll need to add it as a custom network.

<Steps>

{<h3>Open the OP Goerli connection link</h3>}

Click [this link](https://chainid.link/?network=optimism-goerli) to open the OP Goerli connection link.
This link will show you the connection details for OP Goerli and give you the option to add it to MetaMask.

{<h3>Click "connect"</h3>}

Click the "connect" button to add OP Goerli to MetaMask.
...
Click "Approve" to add OP Goerli to MetaMask.

{<h3>Switch to the OP Goerli network</h3>}

After you approve the network, MetaMask will automatically try to switch to the OP Goerli network.
Click "Switch network" to continue.

</Steps>

## Get ETH on OP Goerli

You'll need some ETH on OP Goerli to pay for the [gas fees](https://ethereum.org/en/developers/docs/gas/) associated with deploying a smart contract.
You can use the [Optimism Superchain Faucet](https://app.optimism.io/faucet) to get some free ETH on OP Goerli.

<Callout>
Having issues with the Optimism Superchain Faucet?
You can try using [other available OP Goerli faucets](/builders/tools/faucets) instead.
</Callout>

## Check Your Balance

After you get some ETH on OP Goerli, you can check your balance in MetaMask.
Make sure that your balance has updated before continuing.
If you don't see any balance, double check that your MetaMask is connected to the OP Goerli network.
...
Now that you've written your first contract, you can deploy it to OP Goerli.
...
This is useful for testing, but you'll need to change your environment to deploy to OP Goerli for this tutorial.
...
Click the orange "Deploy" button to deploy your contract to OP Goerli.
...
OP Goerli is relatively fast, so transactions should confirm within just a few seconds.
...
Congrats, you've successfully deployed and interacted with your first smart contract on OP Goerli!
...
Now that you know how to deploy a smart contract to OP Goerli, you can deploy a smart contract to OP Mainnet in exactly the same way.
If you're up for a challenge, try [creating a token on Goerli and OP Goerli](./standard-bridge-standard-token) and then [using the Optimism SDK](./cross-dom-bridge-erc20) to bridge some tokens between the two networks.
