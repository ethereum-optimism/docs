---
title: Additional Configuration for op-node
lang: en-US
...
# Additional Configuration for `op-node` (Consensus-Layer)
...
This page list additional configuration options for `op-node`. `op-node` implements most rollup-specific functionality as Consensus-Layer, similar to a L1 beacon-node. 
