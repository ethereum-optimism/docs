...
<Callout type="info">
  Even though the Docker image for the Execution Engine is called `op-geth`, the actual binary is still called `geth` in order to minimize differences between `op-geth` and `go-ethereum`. You can see the difference [here](https://op-geth.optimism.io/).
</Callout>

`op-geth` stores its state in a database that requires initialization. Initialization is done one of three ways, depending on which network you're deploying:
