...
Node log levels determine the verbosity of log messages, allowing operators to filter
messages based on importance and detail. The log levels for geth (used in Optimism) are
as follows:

1. Silent (0): No log messages are displayed. This level is rarely used as it provides no
feedback on the node's status.

2. Error (1): Only error messages are displayed. Use this level to focus on critical issues
that need immediate attention.

3. Warn (2): Displays error messages and warnings. This level helps to identify potential
problems that might not be immediately critical but require attention.
...
To set the log level, use the --verbosity flag when running the geth command. For
