...
lang: en-US
description: Learn how to use snap sync on your node.
---
...
* For OP Mainnet users, this is a significant upgrade because users will be able to sync without having to download the [bedrock datadir](snapshots).
...
For snap sync, all `op-geth` nodes should be ran with public IP addresses, except for sequencers and transaction ingress nodes.
</Callout>
...
*   **Option 3:** Continue using current snap sync approach (before Ecotone) where `op-node` reads from L1 and inserts blocks into `op-geth`. 
...
* maybe link to a specific node operator tutorial??