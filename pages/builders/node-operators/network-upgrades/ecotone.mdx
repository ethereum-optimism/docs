...
*   For node operators of custom chains not included in the `superchain-registry`, the activation dates can be set in the `rollup.json` (set `ecotone_time`) or via overrides (CLI) in both `op-node` and `op-geth` help. These will need to be set on `op-node` and `op-geth` for the sequencer and all other nodes.
...
There are several blob archiver options for archive nodes. Choose one of the following options:
