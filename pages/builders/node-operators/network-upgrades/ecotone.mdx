...
1.  Enable [`—syncmode=execution-layer`](../management/configuration#configuring-op-node) on `op-node`. This essentially tells `op-node` not to sync from L1 Data because the execution layer has been configured to sync on its own.
...
*   Check that Ecotone time is set to `activation time stamp` in the op-node logs
*   Check that Ecotone and Cancun time is set to `Ecotone time stamp` in the op-geth logs
