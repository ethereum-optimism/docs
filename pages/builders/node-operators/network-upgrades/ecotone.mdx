...
<Callout type="warning">
  If you update to the mainnet release and you have no L1 beacon value set, your node will crash.
</Callout>
...
1.  Enable [syncmode to EL Sync](../management/configuration#configuring-op-node) on `op-node`. This essentially tells `op-node` not to sync from L1 Data because the execution layer has been configured to sync on its own.
