...
1.  The Rollup Node, which is responsible for deriving blocks from L1 and passing them to the Execution engine. It also participates in a peer-to-peer network that synchronizes unsubmitted blocks as the sequencer creates them.
2.  The Execution Engine, which is responsible for executing the blocks it receives from the rollup node and storing state. It also exposes standard JSON-RPC methods to query blockchain data and submit transactions to the network.
...
The following are recommended minimum system requirements to run OP Mainnet:
...
They'll help you get a head start deploying your first OP node.
...
*   Now that you've got your node up and running, check out the [Node Metrics and Monitoring Guide](./management/metrics) for health monitoring.
