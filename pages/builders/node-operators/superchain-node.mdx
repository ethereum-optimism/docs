---
title: How to Run a Node Within the Superchain
lang: en-US
...
# How to Run a Node Within the Superchain
...
<Steps>
  ...
  *   **Node Architecture**: OP Stack rollup nodes use the rollup node and execution client and can also support legacy geth. For more details, see the [Node Architecture](/builders/node-operators/architecture) guide.
  ...
  *   **Option 1:** Follow the [Running a Node with Docker](/builders/node-operators/tutorials/node-from-docker) tutorial, which walks you through the complete process of **building** and **running** your OP Stack rollup node.
...
<Steps>
  {<h3>Set Working Base Config for OP Stack Nodes</h3>}
...
<Steps>
  ...
  *   Update node [log level](https://docs.optimism.io/builders/node-operators/management/configuration#loglevel) based on individual needs. For more details, see the guide on [Geth Logs](https://geth.ethereum.org/docs/fundamentals/logs).
</Steps>
...
<Steps>
  ...
  *   Optionally, you can customize the metrics port and address via the `--metrics.port` and `--metrics.addr` flags, respectively.
  ...
  <Callout type="warning">
  The following steps are intended for `op-geth` instead of `op-node`, so you will need to tweak it to work for rollup nodes running within the Superchain network.
  </Callout>
