...
<Callout>
Data directories are **not required** for nodes using [snap sync](/builders/node-operators/management/snap-sync) but are still required for archive nodes and in instances when you need to trace the entire chain.
If you run a node using snap sync, then you can safely skip this section.
</Callout>
