...
### Snap Sync Synchronization

You will see these `op-node` logs at the start of snap sync:
...
1.  `op-geth` log something like this as it is downloading the headers:

```text
lvl=info msg="Syncing beacon headers" downloaded=116775778 left=1162878 eta=53.182s 
```

2.  For the second stage, `op-geth` will log the following:
