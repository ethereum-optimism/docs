...
*   Set `syncmode=EL` on `op-node` if you don't set `syncmode=full` here on op-geth.
*   For archive nodes, set `syncmode=full` and `gcmode=archive`. 
...
{<h3>Initialize op-geth with the datadir </h3>}

{<h3>Configure op-geth differently </h3>}
