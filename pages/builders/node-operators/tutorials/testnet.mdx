...
<Callout>
The genesis file **may not be required** for nodes using [snap sync](/builders/node-operators/management/snap-sync) and other specific use cases, so modified instructions are provided at that step.
</Callout>
...
*   For nodes on OP Sepolia and nodes using [snap sync](/builders/node-operators/management/snap-sync): Set `--op-network=op-sepolia` flag on `op-geth`
*   For all other nodes and nodes without a named network: download the genesis file from the OP Sepolia networks page
