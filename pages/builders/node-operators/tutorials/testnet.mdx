...
TODO: Callout that `syncmode=EL` should be set on op-node if we don't set `syncmode=full` here.
TODO: Callout that we can set `syncmode=full` + `gcmode=archive` to get an Archive mode. The default settings means full node.
