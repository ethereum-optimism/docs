...
*   L1 Gas Cost changes:
    *   `getL1GasUsed` method of the `GasPriceOracle` contract ([spec 1](https://specs.optimism.io/fjord/predeploys.html#l1-gas-usage-estimation)) is being deprecated
    ...
    *   `L1GasUsed` field of the transaction receipt ([spec 1](https://specs.optimism.io/fjord/exec-engine.html)) is being deprecated

## For Wallets and Front-End Developers

The proposed Fjord upgrade to the OP Stack and OP Mainnet changes the formula for estimating the [L1 Data Fee](https://github.com/ethereum-optimism/docs/blob/main/stack/transactions/fees#l1-data-fee) component of the [OP Stack Transaction Fee](https://docs.optimism.io/stack/transactions/fees).
...
Changes to the L1 Data Fee formula may affect your application if you are computing this fee component on your own. It's strongly recommended that you use [existing tooling](https://docs.optimism.io/builders/app-developers/transactions/estimates#tooling) to estimate transaction fees instead of computing them yourself.
...
<Steps>
  ...
  An onchain transaction will be required to update the scalar for Fjord. This needs to be prepared days in advance before the activation to ensure chain operators don't operate at a loss when Fjord activates. See the [Ecotone encoding tool](https://github.com/ethereum-optimism/optimism/tree/develop/op-chain-ops/cmd/ecotone-scalar) for more details.
  ...
  *   Set `BaseFeeScalar` and `BlobBaseFeeScalar` values based on the [Fjord calculator.](https://docs.google.com/spreadsheets/d/1V3CWpeUzXv5Iopw8lBSS8tWoSzyR4PDDwV9cu2kKOrs/edit#gid=186414307)
  ...
  <Callout type="warning">
    If you are operating an OP Chain that has an entry in the [`superchain-registry`](https://github.com/ethereum-optimism/superchain-registry/blob/main/superchain/configs/chainids.json), the Fjord activation date is part of the `op-node` and `op-geth` nodes. So, no action is needed for the sequencer after preparing the `SystemConfig` **t**ransaction. Please skip to [Step 3: Prepare Batcher](https://www.notion.so/Fjord-notice-page-brief-explainer-added-to-readme-section-of-docs-c6b4a7f9f4044baba6bafb6a43ad8846?pvs=21).
  </Callout>

  Chain operators not in the Superchain Registry will need to manually configure the [activation timestamp](https://github.com/ethereum-optimism/superchain-registry/blob/main/superchain/configs/mainnet/superchain.yaml). You have two configuration options for your sequencer node:
...
<Steps>
  ...
  *   [`op-node`](https://github.com/ethereum-optimism/optimism/releases/tag/op-node%2Fv1.7.7-rc.1)
  ...
  <Callout type="warning">
    If you are operating a node for an OP Chain that has an entry in the [`superchain-registry`](https://github.com/ethereum-optimism/superchain-registry/blob/main/superchain/configs/chainids.json), the Fjord activation date is part of the `op-node` and `op-geth` nodes. So, no action is needed for the sequencer after upgrading to the latest release. Please skip to [Step 3: Verify Your Configuration](https://www.notion.so/Fjord-notice-page-brief-explainer-added-to-readme-section-of-docs-c6b4a7f9f4044baba6bafb6a43ad8846?pvs=21).
  </Callout>
