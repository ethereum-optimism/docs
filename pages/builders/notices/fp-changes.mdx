...
<Callout type="warning">
  *   Fault Proofs are scheduled to launch on Mainnet **Summer 2024**.
  *   Withdrawals put in place before the upgrade will need to be resubmitted post-upgrade.
  *   **Withdrawal times will be longer.** Withdrawals between OP Sepolia and Sepolia will no longer be instant due to the Fault Proofs mechanism.
</Callout>
...
The proposed Fault Proof upgrade requires developers to enable Fault Proof changes before the Mainnet release. This impacts bridges, centralized exchanges, and custom solutions that use withdrawals.
