...
You'll find all of the important information about OP Mainnet, as well as any test networks, on [our Networks page](/op-mainnet/op-networks).
You can choose to connect to OP Mainnet via our rate-limited public endpoints, [private endpoints from infrastructure providers](/op-mainnet/op-networks), or [by running your own node](/node-operators/building-node/intro).
...
In OP Mainnet transaction fees include both an [L1 data fee](/op-stack/transaction-fees/overview#estimating-the-l1-data-fee) and an [L2 execution fee](/op-stack/transaction-fees/overview#the-l2-execution-fee). 
...
[The EIP 1559 parameters](/op-stack/differences#pre--EIP--155-support) have different values in OP Mainnet (and many other OP Stack chains) than those on L1 Ethereum.
