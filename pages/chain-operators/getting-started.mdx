...
Hello! This guide is meant to help you kick off your OP Stack journey by taking you through the process of spinning up your very own OP Stack chain on the Ethereum Goerli testnet. 
...
To see your rollup in action, you can use the [Optimism Mainnet Getting Started tutorial](/op-mainnet/getting-started). Follow these steps:

///UPDATE: tutorial repo no longer exists
...
If a `CREATE2` collision occurs during the L1 contract deployment step of spinning up an op-stack rollup, you might see the following error due to the environment variable:

```
ERROR apply:ext: evm::cheatcodes: non-empty stderr args=["bash", "-c", "jq -r '.transactions[] | select(.contractAddress == \"0x581f62c0901d4dce2553cb6140dBb4CeE533d834\") | select(.transactionType == \"CREATE\" or .transactionType == \"CREATE2\")' < /shared-contracts-bedrock/broadcast/Deploy.s.sol/31337/run-latest.json"] stderr="bash: line 1: /shared-contracts-bedrock/broadcast/Deploy.s.sol/31337/run-latest.json: No such file or directory\n"
```
The solution is to set the `IMPL_SALT` environment variable to something random. See the [`.envrc.example`](https://github.com/ethereum-optimism/optimism/blob/7591f192ddf23b55809d7a1830775f627a1d5307/packages/contracts-bedrock/.envrc.example) file in `contracts-bedrock` for more information. 
