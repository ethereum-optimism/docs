...
2.  Verify `L1BlockNumber` works correctly.
...
3.  Get the current implementation for the contract.
...
4.  Change the implementation to the zero address

    ```sh
    cast send --private-key $PRIVKEY $PROXY_ADMIN "upgrade(address,address)" $L1BLOCKNUM $ZERO_ADDR
    ```

5.  See that the implementation is address zero, and that calling it fails.
...
6.  Fix the predeploy by returning it to the previous implementation, and verify it works.
