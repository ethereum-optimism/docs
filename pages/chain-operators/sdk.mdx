...
If you followed the directions in [Getting Started](/getting-started-op-stack), the contract addresses are in `.../optimism/packages/contracts-bedrock/deployments/getting-started`, which you created when you deployed the L1 contracts.
...
3.  Create the [providers](https://docs.ethers.org/v5/api/providers/) and [signers](https://docs.ethers.org/v5/api/signer/).
...
4.  Create the L1 contracts structure.
...
5.  Create the data structure for the standard bridge.
...
6.  Create the [`CrossChainMessenger`](https://sdk.optimism.io/classes/crosschainmessenger) object.
...
3.  Get the balances after the transfer.
...
4.  See that the L1 balance changed (probably by a lot more than 1 gwei because of the cost of the transaction).

    ```js
    (balances0[0]-balances1[0])/1e9
    ```

5.  See that the L2 balance changed (it might take a few minutes).
