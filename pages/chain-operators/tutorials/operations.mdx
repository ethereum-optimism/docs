...
6.  To enable L2 nodes to synchronize directly, rather than wait until the transactions are written to L1, turn on peer to peer synchronization.
    If you already have peer to peer synchronization, add the new node to the `--p2p.static` list so it can synchronize.

7.  Start `op-geth` (using the same command line you used on the initial node)

8.  Start `op-node` (using the same command line you used on the initial node)
