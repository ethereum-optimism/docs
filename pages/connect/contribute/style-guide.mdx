...
The right sidebar, also known as the page TOC, is created automatically for all the H2 and H3 headings on a page.
...
File paths should be written the same way as code snippets when used **within** a docs page, so they are easier to read. File paths should be wrapped in backticks.
...
*   Use language the audience understands. It is difficult to avoid jargon in the web3 space, but we can do a few things to improve language comprehension for users by doing a few things, such as:
