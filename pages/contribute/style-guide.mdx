...
The folder structure for the [docs.optimism.io](https://github.com/ethereum-optimism/docs) repository is organized into several high-level categories under the main `pages` folder such as `op-stack`, `op-mainnet`, `node-operators`, `dapp-developers`, `chain-operators`, `cex-wallet-developers`, `resources`, `contribute`, and `tools`.
...
- Use sentence case for body content and short phrases, even when the content is a link. Sentence case means you only capitalize the first letter of the sentence. <br />
    **Example:** If you're trying to figure out how to do something specific as a node operator, you might search our collection of [tutorials](/node-operators/overview) or [suggest a new one](https://github.com/ethereum-optimism/docs/issues).
...
- Use the correct callout type based on the type of issue: a) info/general, b) warning, c) error. Our documentation platform supports 4 different callout types.
    ...
    - Warning callouts should be used to indicate important info, such as as when a product or code will be deprecated.
        <Callout type="warning">
        **Example**:<br />
...
- **Example (imported)**:
![Import Img Example](public/img/guides/contribute/import-img.png)
...
- **Example of video with title for alt-text:** ![Embed Video with Alt-text](public/img/guides/contribute/video-title.png)
...
- Use absolute or relative links when linking across pages in the site. Absolute links are cleaner and easier to work with when pages are nested, so they are the recommended option.
    ...
    **Examples**: absolute link `(/op-stack/protocol/deposit-flow)`  versus relative link `(../../protocol/deposit-flow)`
...
- Use the exact title of the page when linking to it in a sentence, whenever possible, and display it in title case. The link should use default styling with no other formatting (bold, italics, quotations).
    ...
    **Example**: Be sure to check out the [OP Stack Getting Started Guide](/op-stack/getting-started) to get up to speed.
...
Content types help manage technical content by defining the purpose and common structure for each file type. These can also be thought of as taxonomies for technical documentation. All content types used in these technical docs have attributes or properties, as defined below.

### Guides

Guides are **learning-oriented** pages that use simple language to explain complex features, such as how a product works. They are great for on-boarding beginning or novice developers. For developer-focused guides, it is best practice to include illustrations, diagrams, and code samples whenever possible to help explain concepts.
...
Overviews are both **information-oriented** and **task-oriented** pages that serve as the landing page for a particular group of related documentation pages; often used at the top-level of a directory, such as `/node-operators/overview`. When done well, overviews accomplish two essential tasks for users:  
...
Tutorials are **task-oriented** pages or videos that include practical steps for completing a task, activity, or objective. Tutorials are more interactive and hands-on than other technical documentation, often including practical examples, exercises, and demonstrations to help developers learn by doing.
...
Troubleshooting guides are both **learning-oriented** and **task-oriented** pages that list common problems a developer might encounter while using a product or service, identifies the symptoms, and offers solutions to these problems. It is important to accurately capture symptoms produced by the system or interface (e.g., error messages, unexpected page refresh/reload, spinning wheel, etc.), so developers know if their system response aligns with one of the common problems identified in the troubleshooting guide.
...
- next steps section: links to related tutorials, guides, troubleshooting, etc.

Technical references often include more links throughout the document than other content types, often linking to other technical references, guides, tutorials, glossary definitions, etc. Since the purpose of technical reference material is to educate developers on a deeper level about the topic of their choosing, this is a common and expected practice and is a good indiction of a strong technical reference.
