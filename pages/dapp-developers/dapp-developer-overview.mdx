...
If you're brand new to OP Mainnet, try starting with the guide on [deploying a basic contract](/getting-started-op-mainnet).
It'll get you familiar with the core steps required to get a contract deployed to the network.
Luckily, OP Mainnet is [EVM equivalent](https://medium.com/ethereum-optimism/introducing-evm-equivalence-5c2021deb306), so it's similar to deploying a contract to Ethereum with a [few small differences](/differences).
...
OP Mainnet even uses this same message-passing infrastructure under the hood of the Standard Token Bridge.
