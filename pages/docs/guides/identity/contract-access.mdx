...
For more context on the AttestationStation visit the [identity overview in our developer documentation](overview.mdx).
...
## Setup
<Steps>
### Step 1: Use `yarn` to download the packages you need
...
### Step 2: Copy `.env.example` to `.env` and modify the parameters
...
### Step 3: Enter the hardhat console
...
### Step 4: Attach to the contract on OP Goerli
...
3. If the raw key is longer than 31 characters, use a different key (one that has the least significant bit turned on so it won't appear to be a regular key).
...
### Step 3: Find your transaction on Etherscan
- If you want to see the key, you can use the hash to find your transaction on Etherscan (or just [use my transaction](https://goerli-explorer.optimism.io/tx/0x0e77a32b2558f39e60c3e81bd6efd811cf4b3bd80a4f666d042a221ea63c93ab))
...
- Key
<Steps>
### Step 1: Read the value for the attestation you just created
...
### Step 2: Check the attestation for a different address to see that the default is false
...
### Step 3: Read an attestation created by a different user (this one is a grade, so it's a string)
...
<Steps>
### Step 1: Create a key that includes the two fields we need to check for equality.

   ```js
   event2key = e => `${e.args.key}-${e.args.creator}`
   ```

### Step 2: Create a function that updates history unless it finds the history already includes
   newer info
...
<Steps>
### Step 1: Deploy `AttestationProxy`
...
### Step 2: Create an attestation and sign it
...
### Step 3: Send the attestation to `AttestationProxy`.
...
### Step 4: Make a note of the address of your attestation proxy

   ```js
   attestationProxy.address
   ```

### Step 5: View the attestation station contract on OP Goerli
- View the [attestation station contract on OP Goerli](https://goerli-optimism.etherscan.io/address/0xEE36eaaD94d1Cc1d0eccaDb55C38bFfB6Be06C77#internaltx) to see the new attestation.
...
### Step 6: Confirm the creator of the attestation is indeed the proxy address
- Click the transaction hash and then the **Logs** tab to see that the creator of the attestation is indeed the proxy address.

### Step 7: Get the Signer
- The mnemonic in `.env` can generate multiple signers.
...
### Step 8: Change signing authority to `otherSigner`
...
### Step 9: Try to submit an attestation signed by yourself again, see that it now fails.

   ```js
   tx = await attestationProxy.attest(goatAddr, historyKey, ethers.utils.toUtf8Bytes("A+"), sig)
   ```

### Step 10: Get `otherSigner`'s signature and resubmit the transaction. See that it is successful.
...
Step 11: Go back to [attestation station contract on OP Goerli](https://goerli-optimism.etherscan.io/address/0xEE36eaaD94d1Cc1d0eccaDb55C38bFfB6Be06C77#internaltx) to see the new attestation.
