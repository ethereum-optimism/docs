...
Get the L1 contract addresses, and [provide them to the SDK](/op-stack/build/sdk#getting-contract-addresses).
...
### Step 4: Copy `.env.example` to `.env` and edit it
...
<Steps>
### Step 1: Submit the withdrawal transaction on OP Mainnet (or OP Goerli)
### Step 2: Wait until the state root with the withdrawal is published (and the status changes to `optimismSDK.MessageStatus.READY_TO_PROVE`)
### Step 3: Submit the proof on L1 using `crossChainMessenger.proveMessage()`
### Step 4: Wait the fault challenge period
  - When this period is over, the status becomes `optimismSDK.MessageStatus.READY_FOR_RELAY`
### Step 5: Finalize to cause the actual withdrawal on L1 using `crossChainMessenger.finalizeMessage()`
</Steps>

[You can read more about this in the documentation](/op-stack/bedrock/changes#two-phase-withdrawals).
