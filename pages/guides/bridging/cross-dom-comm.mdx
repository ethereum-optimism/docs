...
[You can read more details about this process here](messaging).
...
### Step 2: Copy `.env.example` to `.env` and edit it
 ...
 - Set `OP_GOERLI_ALCHEMY_KEY` to the key for the OP Goerli app
...
The fault challenge window starts after you do this, so it's best to do it as early as possible. [You can read more about this in the documentation](/op-stack/bedrock/changes#two-phase-withdrawals).
...
<Steps>
### Step 1: Install the `@eth-optimims/sdk` library (assuming you already have Node.js and yarn)
...
### Step 6: In the block explorer, [view the event log](https://goerli-explorer.optimism.io/address/0xE8B462EEF7Cbd4C855Ea4B65De65a5c5Bab650A9#events)
   - Notice that the `xorigin` value is the controller address.
   - The `user` value is your user's address, but that one is provided as part of the message.
</Steps>
