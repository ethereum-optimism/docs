...
[Click here for the OP Mainnet endpoints](/op-stack/op-networks). You can choose between our public endpoints, which are rate limited, and todo:[endpoints from infrastructure providers](op-stack/op-networks). Given rate throughput limits, we recommend using a private rpc provider for both mainnet and testnet use cases. 
...
You can read more about [transaction fees](/guides/transaction-fees/overview).
...
When a user wants to withdraw the assets back to L1, the bridge burns the asset on L2 and releases it to the user on L1. If you want to use this gateway automatically, [follow this tutorial for ETH](/guides/bridging/cross-dom-bridge-eth) or [this one for ERC-20 tokens](/guides/bridging/cross-dom-bridge-erc20).
...
If there is no need for custom business logic, you can [follow the steps in this tutorial](/guides/bridging/standard-bridge-standard-token).
If you need to implement some kind of custom logic, [see this tutorial](/guides/bridging/standard-bridge-custom-token).
