...
todo(update internal links):[Click here for the OP Mainnet endpoints](../useful-tools/networks.md). You can choose between our public endpoints, which are rate limited, and todo(upate internal links):[endpoints from infrastructure providers](../useful-tools/networks.md). Given rate throughput limits, we recommend using a private rpc provider for both mainnet and testnet use cases. 
...
todo(update internal links):[You can read more about this subject here](../developers/build/transaction-fees.md).
