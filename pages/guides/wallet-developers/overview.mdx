...
OP Mainnet shares the [Ethereum JSON-RPC API](https://eth.wiki/json-rpc/API) with only [a few minor differences](/guides/node-operators/json-rpc).
You'll find all of the important information about OP Mainnet, as well as any test networks, on [our Networks page](/op-stack/op-networks).
You can choose to connect to OP Mainnet via our rate-limited public endpoints, [private endpoints from infrastructure providers](/op-stack/op-networks), or [by running your own node](/guides/node-operators/building-node/intro).
...
In OP Mainnet transaction fees include both an [L1 data fee](/guides/transaction-fees/overview#estimating-the-l1-data-fee) and an [L2 execution fee](/guides/transaction-fees/overview#the-l2-execution-fee). 
...
[The EIP 1559 parameters](/op-stack/bedrock/differences#eip-1559) have different values in OP Mainnet (and many other OP Stack chains) than those on L1 Ethereum.
