...
<Steps>
  ...
  *   Have the same storage layout as the ERC20 contracts they replace (in the case of chain A).
  ...
  1.  This `src/InteropToken.sol` is used for contract upgrades. 
  ...
  1.  Deploy `InteropToken` on both chains, with the bridge address.
  ...
  1.  Update the proxies to the new implementations.
  ...
  1. Deploy the bridges with the correct configuration.
  ...
  1. Inform the proxy beacons about the new addresses of the bridge implementation contracts.
