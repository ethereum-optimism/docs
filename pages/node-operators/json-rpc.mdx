...
Some methods have been changed after protocol upgrade:
- Use `eth_getBlockByNumber` in a batch request instead of `eth_getBlockRange`.
- Do not use `rollup_getInfo`. None of the information returned by this method exists on Bedrock, so there is no replacement for this method.
