...
Since the Ecotone upgrade, the Optimism protocol prices L2 transactions using a function that incorporates L1 base fee, L1 blob base fee, and estimated data availability costs. The so-called "Ecotone scalars" are operator-controlled parameters stored in the SystemConfig contract and can be used to tune the target profit margin.
