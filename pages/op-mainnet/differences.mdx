...
| `DIFFICULTY` | `block.difficulty` | Random value. As this value is set by the sequencer, it is not as reliably random as the L1 equivalent. [You can use an oracle for randomness](/tools/oracles#verifiable-randomness-function-vrf). |
...
[Transaction costs on OP Mainnet](/guides/transaction-fees/overview) include an [L2 execution fee](/guides/transaction-fees/overview#the-l2-execution-fee) and an [L1 data fee](/guides/transaction-fees/overview#the-l1-data-fee). 
...
See the full list of [custom JSON-RPC methods](/guides/node-operators/json-rpc) for more information.
