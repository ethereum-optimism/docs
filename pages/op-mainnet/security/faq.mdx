...
As part of the OVM 2.0 upgrade, the **Optimism fault proof mechanism had to be temporarily disabled**. This means that users of the OP Mainnet network currently need to trust the Sequencer node (run by Optimism Foundation) to publish valid state roots to Ethereum. **You can also read more about our [security model](/resources/security-policy)**. 
...
[OP Mainnet has one of the biggest bug bounties (ever)](/resources/security-policy).
...
[View the Security Policy for details about reporting vulnerabilities and available bug bounty programs](/resources/security-policy)
</details>