...
OP Mainnet’s migration to Bedrock has concluded and the Bedrock sequencer has started up! Please visit the docs for images and data directories for [node operators](/op-stack/op-networks#parameters-for-node-operators).
...
While we recommend using the **`legacy-geth`** archival datadir we upload **[once it's ready](/op-stack/op-networks#links)**, given the size of the archival data disk and the time 
...
Throughout the migration, the [Optimism Status Page](https://status.optimism.io/) will be updated with the latest migration status and any important information for our partners and node operators.
  ...
  | Build and Push Docker Images | This step will release the op-node image for OP Mainnet in the [OP Mainnet section of the Developer Docs](/op-stack/op-networks#optimism-mainnet) (op-geth image is already available)
...
This section assumes that you have read and understood our **[Node Operator Guide](/guides/node-operators/node-operator-guide)**. Please read that first if you have not already.
...
On upgrade day, we will provide node operators with the following information in the [OP Mainnet network section](/op-stack/op-networks#optimism-mainnet). To be notified as soon as these are available, watch the [Optimism Status Page](https://status.optimism.io/):
...
1. Initialize `op-geth`'s data directory using the upgraded genesis state from the provided URL. See the **[Initialization via Data Directory](/guides/node-operators/node-operator-guide#initialization-via-data-directory)** section of the Node Operator Guide for more information.
...
3. If you are running an archive node, initialize Legacy Geth's data directory using the legacy genesis state from the provided URL. See the **[Initialization via Data Directory](/guides/node-operators/node-operator-guide#initialization-via-data-directory)** and **[Legacy Geth](/guides/node-operators/node-operator-guide#legacy-geth)** sections of the Node Operator Guide for more information.
...
The L1 gas price formula will not be changed, but the we expect to reduce [the dynamic overhead multiplier](/guides/transaction-fees/overview#the-l2-execution-fee) significantly (by ~ 40%) because of the better compression. This is the main cost of Optimism transactions.
