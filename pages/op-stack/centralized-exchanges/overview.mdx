...
[Click here for the OP Mainnet endpoints](/op-mainnet/op-networks). You can choose between our public endpoints, which are rate limited, and todo:[endpoints from infrastructure providers](op-mainnet/op-networks). Given rate throughput limits, we recommend using a private rpc provider for both mainnet and testnet use cases. 
...
You can read more about [transaction fees](/op-stack/transaction-fees/overview).
