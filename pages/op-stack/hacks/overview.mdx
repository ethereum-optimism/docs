...
OP Stack Hacks create blockchains that aren’t exactly OP Stack, and may be insecure. Hacked OP Stack chains can break key invariants that are required to interoperate with [the Optimism Superchain](/op-stack/explainer). **Developers of chains that wish to interoperate with [the Optimism Superchain](/op-stack/explainer) should *not* include any hacks**. When in doubt, stick with the official components within [the current release of the OP Stack](/op-stack/getting-started#the-op-stack-today).
