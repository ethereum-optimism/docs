...
### Step 2: For using Foundry, set `ETH_RPC_URL`.

    ```sh
    export ETH_RPC_URL=$GOERLI_RPC
    ```

### Step 3: Check the balance of the ADMIN account.
...
### Step 5: Pause the portal.

   ```sh
   cast send --private-key $PRIV_KEY $PORTAL_ADDR "pause()"
   ```

### Step 6: Send a deposit to L2.
...
### Step 7: Wait ten minutes and see which transaction(s) have been relayed using the [SDK](/tools/javascript/sdk). 
   Use [`getMessageStatus`](https://sdk.optimism.io/classes/crosschainmessenger#getMessageStatus) to get the information.


### Step 8: Unpause the portal.
