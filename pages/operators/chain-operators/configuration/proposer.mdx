...
the output roots (a proposal) to the L1, to make it available for verifiers. Withdrawals to an L1 must reference an output root.
If the chain is running permissioned fault proofs only the [designated propser](https://docs.optimism.io/superchain/privileged-roles#proposer) can submit output roots.
With [permissionless fault proofs](https://docs.optimism.io/stack/fault-proofs/explainer), anyone can make a proposal.
