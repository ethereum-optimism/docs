...
This guide provides step-by-step instructions for spinning up a sequencer node after deploying L1 smart contracts for your OP Stack chain with [`op-deployer`](/chain-operators/tools/op-deployer).
...
*   Discover how to [deploy chains with op-deployer](/chain-operators/tools/op-deployer) for standardized OP Stack deployments.
*   Learn how to configure and deploy the [batcher](/chain-operators/guides/configuration/batcher) to submit transaction data to L1.
*   Set up the [proposer](/chain-operators/guides/configuration/proposer) to submit output roots for withdrawals.
*   Explore chain operator [best practices](/chain-operators/guides/best-practices) for production deployments.
