  ...
  services:
    op-geth:
      ...
      command: |
        ...
        # ... other geth flags, see the build-with-source section for all of them
    ...
    op-node:
      ...
      command: |
        ...
        # ... other op-node flags, see the build-with-source section for all of them
  ```
...
For this basic sequencer setup, you only need the sequencer private key.
...
# Private keys - Only sequencer key needed for this
...
./data/op-geth/build/bin/geth init --datadir=./data/op-geth --state.scheme=hash ./genesis.json
```
...
If you started with `SEQUENCER_STOPPED=true`, enable sequencing:
