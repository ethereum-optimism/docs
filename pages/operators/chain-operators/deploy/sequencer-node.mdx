...
This guide provides step-by-step instructions for spinning up a sequencer node after deploying your L1 smart contracts for your OP Stack chain with [`op-deployer`](/operators/chain-operators/tools/op-deployer).
...
Jump to the [complete docker setup](/operators/chain-operators/deploy/sequencer-node#complete-docker-setup-guide)

#### Pull the required images

```bash
# Pull this op-node image
docker pull us-docker.pkg.dev/oplabs-tools-artifacts/images/op-node:v1.13.3

# Pull this op-geth image  
docker pull us-docker.pkg.dev/oplabs-tools-artifacts/images/op-geth:v1.101511.0
```
...
```bash
./build/bin/geth [other other arguments]
```
...
docker run --rm \
  -v $(pwd):/workspace \
  -v $(pwd)/op-geth-data:/workspace/op-geth-data \
  --network host \
  ...
  --authrpc.vhosts="*" \
  --authrpc.jwtsecret=/workspace/$JWT_SECRET \
  --syncmode=full \
  --gcmode=archive \
...
docker run --rm \
  ...
  --l2.jwt-secret=/workspace/$JWT_SECRET \
...
<details>
  ...
  2.  **Initialize op-geth with Docker before starting services:**

  ```bash
  # Initialize op-geth using Docker
  docker run --rm \
    -v $(pwd)/op-geth-data:/op-geth-data \
  ...
  4.  **Directory structure for Docker approach:**
