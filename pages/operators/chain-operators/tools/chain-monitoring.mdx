...
For verifying the legitimacy of claims, `op-challenger` relies on a synced, trusted rollup node as well as a trace provider (e.g., [Cannon](/concepts/architecture/fault-proofs/cannon)). See the [OP-Challenger Explainer](/concepts/architecture/fault-proofs/op-challenger) for more information on this service.
