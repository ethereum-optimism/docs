...
<Steps>
  ...
  *   `genesis.json` - L2 genesis file from the `op-deployer` guide
</Steps>
...
<Tabs items={['Using Docker (Recommended)', 'Build from source']}>
    ...
    <Steps>
      ...
      ```bash
      # Create and enter the challenger directory
      mkdir challenger-node && cd challenger-node
      ```
