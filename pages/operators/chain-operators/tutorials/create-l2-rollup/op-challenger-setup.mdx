...
<Tabs items={['Using Docker (Recommended)', 'Build from source']}>
    ...
    <Steps>
      ...
      *   `prestate-proof-mt64.json` and `${PRESTATE_HASH}.bin.gz`: Prestate files required for dispute games (the PRESTATE_HASH comes from running `make reproducible-prestate`)
