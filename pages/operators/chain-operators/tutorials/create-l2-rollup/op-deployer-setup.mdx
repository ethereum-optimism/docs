    ...
    <Steps>
      ...
      *   For Windows: `op-deployer-windows-amd64.exe`
      ...
      ```bash
        ...
        # Extract the compressed tar.gz archive (x = extract, v = verbose, z = gzip, f = file)
        tar -xvzf op-deployer-0.2.6-darwin-arm64.tar.gz

        # Move the extracted file to /usr/local/bin so it can be run globally as a command
        # (Note: right now this command is moving the archive itself, not the extracted binary)
        sudo mv op-deployer-0.2.6-darwin-arm64.tar.gz /usr/local/bin/
      ```
