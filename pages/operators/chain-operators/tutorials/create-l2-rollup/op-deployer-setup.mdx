  ...
  <Tabs.Tab>
    ...
    <Steps>
      ...
      *   For Windows: `op-deployer-windows-amd64.exe`
      ...
      ```bash
      # Step 1: Go to the Downloads folder (where your file is saved)
      cd ~/Downloads

      # Step 2: Extract the tar.gz archive (x = extract, v = verbose, z = gzip, f = file)
      tar -xvzf op-deployer-0.2.6-darwin-arm64.tar.gz

      # Step 3: Move into the extracted folder (adjust if folder name differs)
      cd op-deployer-0.2.6-darwin-arm64

      # Step 4: Make the binary executable
      chmod +x op-deployer

      # Step 5: Remove macOS quarantine attribute (fixes "can't be opened" warning)
      sudo xattr -dr com.apple.quarantine op-deployer

      # Step 6: Move the binary into /usr/local/bin (Intel Macs)
      # For Apple Silicon Macs, you may prefer /opt/homebrew/bin instead
...
<Steps>
  ...
  ```bash
  #Create a directory inside the deployer's directory
  mkdir -p keys
  cd keys
  ```
