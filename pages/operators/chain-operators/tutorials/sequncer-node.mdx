---
title: Setting up an Optimism sequencer node
lang: en-US
...
op-geth embeds upstream geth's version inside its own version as follows: vMAJOR.GETH_MAJOR GETH_MINOR GETH_PATCH.PATCH. For example, if geth is at v1.12.0, the corresponding op-geth version would be v1.101200.0.
...
The sequencer can be initialized in a stopped state using the sequencer.stopped flag. The sequencer can be started using the admin_startSequencer RPC.
