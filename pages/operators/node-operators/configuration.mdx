---
title: Configuration
lang: en-US
description: >-
  Learn about configuration in the Optimism ecosystem. This guide provides
  detailed information and resources about configuration.
content_type: landing-page
topic: node-configuration-resources
personas:
  - node-operator
categories:
  - node-configuration
  - infrastructure
  - system-configuration
  - node-management
  - execution-client
  - consensus-client
  - system-design
  - system-components
is_imported_content: 'false'
---

import { Card, Cards } from 'nextra/components'

# Configuration

This section provides information on node base configuration, consensus layer configuration options (OP Node), and execution layer configuration options (OP Geth). You'll find information to help you understand and work with these topics.

<Cards>
  <Card title="Node base configuration" href="/node-operators/guides/configuration/base" />

  <Card title="Consensus layer configuration options (op Node)" href="/node-operators/guides/configuration/consensus" />

  <Card title="Execution layer configuration options (op Geth)" href="/node-operators/guides/configuration/execution" />
</Cards>
