...
All nodes in the Superchain ecosystem can be run from `op-node` and `op-geth` binaries if they're included in the [Superchain Registry](/chain-operators/reference/superchain-registry). You can specify the type of node you want to run by configuring the `--network` flag on `op-node` and the `--op-network` flag on `op-geth`.
...
<Steps>
  ...
  Refer to the `op-geth` [configuration documentation](/node-operators/guides/configuration/execution) for more detailed information about available options.
...
<Steps>
  ...
  Refer to the `op-node` [configuration documentation](/node-operators/guides/configuration/consensus) for more detailed information about available options.
