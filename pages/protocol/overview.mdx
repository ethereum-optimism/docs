...
<Image src="/img/op-stack/protocol/ethereum-optimism-forever.png" alt="Ethereum and Optimism Forever Doodle" width={400} height={400} />
...
This is important because writing to L1 is [the major cost of OP Mainnet transactions](/op-stack/transaction-fees/overview).
...
See the [developer documentation and examples](/dapp-developers/bridging/standard-bridge) on details on the inner workings of the Standard bridge.
...
You use [`L1CrossDomainMessenger`](/dapp-developers/bridging/cross-dom-comm) or [`L1StandardBridge`](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/contracts/L1/L1StandardBridge.sol).
...
1. Wait for the next output root to be submitted to L1 (you can see this on [the SDK](/tools/javascript/sdk)) and then submit the withdrawal proof using `proveWithdrawalTransaction`.
