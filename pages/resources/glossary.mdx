...
- **Chain Re-Organization**: re-organization, or re-org for short, is whenever the head of a blockchain (its last block) changes (as dictated by
the [fork choice rule][fork-choice-rule]) to a block that is not a child of the previous head. L1 re-orgs can happen because of network conditions or attacks. L2 re-orgs are a consequence of L1 re-orgs, mediated via
...
- **Fork Choice Rule**: used to determine which block is to be considered as the head of a blockchain. On L1,
this is determined by the proof of stake rules. L2 also has a fork choice rule, although the rules vary depending on whether we want the [safe L2 head][safe-l2-head],
the [unsafe L2 head][unsafe-l2-head] or the [finalized L2 head][finalized-l2-head].

- **Priority Gas Auction**: Transactions in ethereum are ordered by the price that the transaction pays to the miner. Priority Gas Auctions
...
- **Sequencing Window**: range of L1 blocks from which a [sequencing epoch][sequencing-epoch] can be derived.
...
- **Sequencing Epoch**: sequential range of L2 blocks derived from a [sequencing window](#sequencing-window) of L1 blocks.
...
- **L1 Origin**: the L1 origin of an L2 block is the L1 block corresponding to its [sequencing epoch][sequencing-epoch].
...
- **Deposited Transaction**: L2 transaction that was derived from L1 and included in a L2 block.
...
- **User-Deposited Transaction** deposited transaction which is derived from an L1 call to the deposit contract and depositing call.
...
- **Finalization Period**: sometimes also called *withdrawal delay* — is the minimum amount of time (in seconds) that
must elapse before a [withdrawal][withrawals] can be finalized. The finalization period is necessary to afford sufficient time for [validators][validator] to make a [fault
proof][fault-proof].
...
Availability][data-availability] for more information on what this means. Ideally, a good data availability provider provides strong *verifiable* guarantees of data availability
...
- **Channel**: sequence of [sequencer batches][sequencer-batch] (for sequential blocks) compressed together. The reason
...
- **L2 Derivation Inputs**: data that is found in L1 blocks and is read by the [rollup node][rollup-node] to construct [payload
...
- **L2 Genesis Block**: first block of the L2 chain in its current version. The state of the L2 genesis block comprises:
...
- **Finalized L2 Head**: highest L2 block that can be derived from *[finalized][finality]* L1 blocks — i.e. L1
...
- **Rollup Node**: node responsible for [deriving the L2 chain][derivation] from the L1 chain (L1 [blocks][block] and their
associated [receipts][receipt]). The rollup node can run either in *validator* or *sequencer* mode.
...
- **L1 Attributes Predeployed Contract**: [predeployed contract][predeploy] on L2 that can be used to retrieve the L1 block attributes of L1 blocks with a given
block number or a given block hash.

[L1 Attributes Predeployed Contract Specification](deposits.md#l1-attributes-predeployed-contract)

- **L2 Output Root**: 32 byte value which serves as a commitment to the current state of the L2 chain.
...
- **Validator**: an entity (individual or organization) that runs a [rollup node][rollup-node] in validator mode.
...
- **Time Slot**: On L2, there is a block every 2 second (this duration is known as the [block time][block-time]).
...
- **Block Time**: L2 block time is 2 second, meaning there is an L2 block at every 2s [time slot][time-slot].
...
- **Unsafe Sync**: process through which a [validator][validator] learns about [unsafe L2 blocks][unsafe-l2-block] from
