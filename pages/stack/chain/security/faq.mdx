...
Although the fault proof game is permissionless, the Optimism Security Council acting as the [Guardian](/stack/chain/security/privileged-roles#guardian) role provides a backstop in case of a failure in the fault proof game.
...
[OP Mainnet has one of the biggest bug bounties (ever)](/stack/chain/security/security-policy).
...
For details about reporting vulnerabilities and available bug bounty programs, see the [Security Policy](/stack/chain/security/security-policy).
