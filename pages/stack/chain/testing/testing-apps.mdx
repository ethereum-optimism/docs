...
It is a best practice to design and run thorough tests across an OP test network, either in your [local development environment](dev-node) or on [the test network](/stack/chain/networks#op-sepolia), depending on your use case.
...
Alternatively, you can connect to our [test network](/stack/chain/networks#op-sepolia) if those contracts are also deployed there (in many cases they are).
