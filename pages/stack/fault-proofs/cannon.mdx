...
separate action(s) needs to be performed. The action(s) to be performed are configured by the user, and can include logging information,
...
Action(s) to be performed are instantiated and checked by [`matcher.go`](https://github.com/ethereum-optimism/optimism/blob/develop/cannon/cmd/matcher.go),
