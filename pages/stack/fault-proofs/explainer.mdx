...
In [Optimistic Rollups like OP Stack Chains](/concepts/architecture/rollup/overview) there is a \~1 week challenge period during which users can challenge a proposal if they believe it to be incorrect.
...
The OP Stack Fault Proof System is [modular in design](/stack/fault-proofs/fp-components#system-design--modularity) and lays the groundwork for achieving a "multi-proof" system. This allows the OP Stack to support multiple proof systems alongside the initial [Cannon](/concepts/architecture/fault-proofs/cannon) proof system.
...
Additionally, the following [security safeguards](/concepts/security/fault-proofs) have been built around the game, as follows:
...
*   See the [Fault Proof Mainnet Security](/concepts/security/fault-proofs) to understand changes to `OptimismPortal` and `FaultDisputeGame` contracts.
...
For detailed guidance on running `op-challenger`, see the [OP-Challenger explainer](/stack/fault-proofs/challenger) and [how to configure challenger for your chain](/operators/chain-operators/deploy/op-challenger).
