...
Additionally, OP Supervisor reads information from L1's consensus layer to know what L2 blocks are unsafe (not written to L1 yet), which are safe (written to L1, but might suffer from a [reorg](https://blog.quicknode.com/understanding-blockchain-reorgs-why-block-numbers-dont-matter-as-much-as-you-think)), and which are finalized (old enough to be immune to reorgs).
...
However, block `n+105` in chain A is unsafe.
Since the new block depends upon it, it is also unsafe.
