...
A cross-chain message refers to any communication sent using Superchain interop. This includes messages sent between different chains within an interop cluster, as well as messages sent on a single chain for interoperable functionality. This capability enables asset transfers that utilize the [SuperchainERC20](superchain-erc20) token standard.
