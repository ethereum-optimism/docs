...
[Reorgs](https://www.alchemy.com/overviews/what-is-a-reorg#what-happens-to-reorgs-after-the-merge) and a few other types of sequencing errors can result in one blockchain having an incorrect view of another.
The main risk is [double spending](https://en.wikipedia.org/wiki/Double-spending), a problem that can happen when an asset appears in two places at once.
...
A double-spend problem occurs when the destination chain receives a valid initiating message, but due to an issue on the source chain, such as a reorg, that initiating transaction is no longer valid. 
When that happens, the tokens are still on the source chain, but they also on the destination chain.

</details>

In Ethereum, incorrect state is usually a transient phenomenon.
On L2, a sequencer can share one block through gossip and then post a different block to L1.
But once the block is posted to L1, that is the correct block for the blockchain.
...
To mitigate the double-spend problem and deliver a low-latency cross-chain experience, OP Stack interop can achieve one-block latency cross-chain transfers through the use of [block safety levels](./explainer#block-safety-levels).
This means users can transfer assets across chains in the Superchain with 1-block latency, and should a reorg happen, both the source and destination transaction would revert preventing a double spend problem.
...
In this case, the block that is written to L1 (let's call it A'<sub>101</sub>) is the valid one, and that causes every dependant block to be recalculated.
...
- The block posted to L1 includes incorret information, for example because it relied on a node on a different blockchain for interop and that node reported incorrect information.
...
If the new block does not contain the same blob, then the sequencer on the chain will know it is missing and repost it.
