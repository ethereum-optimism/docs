...
The [`SuperchainTokenBridge`](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/L2/SuperchainTokenBridge.sol) is an abstraction built on top of the **`L2ToL2CrossDomainMessenger`** that facilitates token bridging using Superchain interop, but requires the token address be deterministic across chains. 
...
*   Token issuers can use other token standards (xERC20, OFT, NTT) and use the `SuperchainTokenBridge` or `L2ToL2CrossDomainMessenger` to benefit from Superchain interop. 
