...
<Steps>
  ...
  1.  Create this file in `packages/contracts/src/LockboxSuperchainERC20.sol`:
      ...
      <details>
        ...
        ```solidity file=<rootDir>/public/tutorials/LockboxSuperchainERC20.sol#L47-L57 hash=ba2c333c4d581d79d4b80d918b183dc8
        ```
        ...
        ```solidity file=<rootDir>/public/tutorials/LockboxSuperchainERC20.sol#L59-L67 hash=4ae43aa03ebf68f5c3a6d08dfcb0a524
        ```
  ...
  1. Actually deploy the contract.

      ```sh
      pnpm contracts:deploy:token
      ```

  1. Get the new token address and store it in an environment variable.
...
<details>
  ...
  1. Lock a quarter token in the lockbox ERC-20 contract.
  ...
  1. See the balances of the user, both original and lockbox, and the balance of the lockbox contract itself.
  ...
  1. Transfer 0.1 token to chain B.
  ...
  1. See the user's balances on both chains.
  ...
  1. Specify the configuration for another user.
  ...
  1. Transfer new tokens to the new user (on chain B) and see that they were actually tansferred.
  ...
  1. As the new user, transfer tokens back to chain A and redeem them.
  ...
  1. See that the second user does not have any more of the new tokens, but does have the original token.
