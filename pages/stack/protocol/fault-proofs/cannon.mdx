...
This documentation will go into detail about the sub-components that make up the offchain Cannon implementation as a whole.
...
witness hashes. Now, in the bisection game, OP-Challenger will provide the generated hashes
...
Before a fault dispute game reaches the execution trace portion of bisection, OP-Program runs as a standalone Go binary. It derives
the L2 outputs from L1 state, which at the output root portion of the bisection game informs OP-Challenger of what actions to take on
the fault dispute game. Then, once the execution trace bisection begins and Cannon is run, OP-Program will be compiled into a MIPS
Executable and Linkable Format (ELF) binary to be run inside of Cannon. Within Cannon is the `mipsevm` that is built to handle the MIPS R3000,
32-bit Instruction Set Architecture (ISA). OP-Program is built such that the same inputs will produce not only the same outputs, but the
same execution trace. This allows all participants in a fault dispute game to run OP-Program within Cannon such that, given the same L2
block state transition, can generate the exact same execution traces which will in turn generate the witness proof for the exact same
instruction to run onchain.

![OP-Program Execution Modes.](/img/op-stack/protocol/op-program-execution-modes.svg)
...
since the root of fully zeroed sub-trees can be computed without actually creating the full-subtree:
...
specifically in `memory.go` is caching the last two pages that have been used. Two pages are cached on account of the `mipsevm` using up
...
Once the execution trace portion of the bisection game begins, OP-Program will be compiled into an ELF file, where each MIPS
instruction within the file will be run within Cannon. However, getting OP-Program into Cannon so that it can be run requires
...
Another step that occurs while loading the ELF file is patching the binary of any incompatible functions. This step is **very
...
[`mips.go`](https://github.com/ethereum-optimism/optimism/blob/develop/cannon/mipsevm/mips.go) implements all of the required MIPS
