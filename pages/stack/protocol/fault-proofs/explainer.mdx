...
Users withdraw ETH and tokens from OP Stack chains like OP Mainnet by submitting a withdrawal proof that shows the withdrawal was actually included inside of the OP Stack chain.
...
In [Optimistic Rollups like OP Stack Chains](/stack/protocol/rollup/overview) there is a \~1 week challenge period during which users can challenge an proposal if they believe it to be incorrect.
...
The OP Stack Fault Proof system is [modular in design](/stack/protocol/fault-proofs/fp-components#system-design--modularity) and lays the groundwork for achieving a "multi-proof" system. This allows the OP Stack to support multiple proof systems alongside the original [Cannon](/stack/protocol/fault-proofs/cannon) proof system.
...
*   See the [Fault Proof Mainnet Security](/stack/protocol/fault-proofs/fp-security) to understand changes to `OptimismPortal` and `DisputeGameFactory` contracts.
...
The Guardian role has the ability to override the system by pausing withdrawals, blacklisting games, or reverting to a permissioned system.
