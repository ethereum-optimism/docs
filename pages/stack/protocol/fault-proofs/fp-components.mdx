...
The first full implementation of the VM generic in the bisection game includes a single MIPS thread context on top of the EVM to execute single instruction within an execution trace generated by `Cannon` and the `op-program`.
