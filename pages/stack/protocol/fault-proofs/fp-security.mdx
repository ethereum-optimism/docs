...
*   Unlike the `L2OutputOracle`, the `DisputeGameFactory` contract is no longer an entirely trusted contract but instead offers users the ability to play a "fault dispute game" in which the correctness of the proposal is determined programmatically.
...
All of the security mechanisms we put in place generally revolve around the possibility that a dispute game may incorrectly finalize and invalid proposal.
