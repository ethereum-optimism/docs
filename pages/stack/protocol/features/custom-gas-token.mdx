...
Aside from setting `useCustomGasToken` and `customGasTokenAddress` during deployment, all other configurations can be set to our standard configuration values: [Standard Config Values](../op-stack/standard-config-values).
