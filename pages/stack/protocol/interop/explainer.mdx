...
Chain operators can add or remove chains from the dependency set through the `SystemConfig`. It is not a requirement that chains are in each other's dependency set. It is permissionless to define a dependency on a chain, so chain operators will be able to choose which chains their chains depend on. 
