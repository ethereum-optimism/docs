...
2.  Transactions are given a timestamp no more than [`max_sequencer_drift`](https://github.com/ethereum-optimism/specs/blob/main/specs/derivation.md#overview) in the future.
3.  Transactions are processed in the order they are received.
4.  Transactions are processed within the [`sequencer_window`](https://github.com/ethereum-optimism/specs/blob/main/specs/glossary.md#sequencing-window).
...
Refer to the [L2 Chain Derivation Specification](https://github.com/ethereum-optimism/specs/blob/main/specs/derivation.md) for a much more detailed explanation of how transactions sent to the `OptimismPortal` contract are processed.
