...
The `AnchorStateRegistry` stores the latest unchallenged "anchor" state for each 
`FaultDisputeGame` type. These anchor states allow new games to start from a recent 
trusted state, reducing offchain computation requirements.
