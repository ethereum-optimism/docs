...
### Step 2: Copy `.env.example` to `.env` and modify the parameters
   ...
   - `OPTIMISM_GOERLI_URL` is the URL for OP Goerli, if you use [a different node provider](/tools/rpc-providers).

   - `OPTIMISM_MAINNET_URL` is the URL for OP Mainnet, if you use [a different node provider](/tools/rpc-providers).


### Step 3: Use Node to run the script
