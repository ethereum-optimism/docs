...
However, there are [a few differences](/op-mainnet/differences), which sometimes require decentralized applications to access Optimism-specific services.

## Optimism-Specific Services
- [Gas costs](/op-stack/transaction-fees/overview)
...
[Our gas costs](/op-stack/transaction-fees/overview) are predominately the cost of writing the transaction to L1, which depends on the transaction size, not the amount of processing required.
...
1. [The JavaScript SDK](https://sdk.optimism.io/). For use when you write JavaScript or TypeScript code, either in the client or a Node.js server.
1. [Offchain, using RPC](/node-operators/json-rpc), which is more complicated but usable from any development stack (Python, Rust, etc.).
