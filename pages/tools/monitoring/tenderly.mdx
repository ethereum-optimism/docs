...
<Steps>
### Step 1: Search for the transaction hash
   ...
   - Click the **Debugger** on the left sidebar.

   - Click in the function trace on a contract where you don't have the source code yet.

   - Click **Fetch the contract from public explorer**.
<Callout>
Note that sometimes it takes a few minutes to fetch the source code.
</Callout>

### Step 2: View the stack trace
   - In the overview you can see the stack trace.
   - The different icons stand for different contract addresses (all implementing the same `Fail` contract in this case).

   ![stack trace](public/docs/op-stack/tutorials/tenderly/stack-trace.png)

### Step 3: View the function trace 
- You can see the function trace, including where exactly in the source the failure was and what happened before that.

   ![function trace](public/docs/op-stack/tutorials/tenderly/func-trace.png)
...
<Steps>
### Step 1: Trace the successful transaction (`0x86b4fc03c1656b64206cdedd1ef840563c1aa32a7ab7c2ca4c2dcb205752848d`)

### Step 2: Select **State Changes** on the left bar to see the state changes caused by the transaction

   ![state changes](public/docs/op-stack/tutorials/tenderly/state-changes.png)

### Step 3: Select **Debugger** on the left bar to see the function trace (which functions ran, which functions called which, etc.), the stack trace (the current funtion and who called it), the source code, and the 

   ![debugger](public/docs/op-stack/tutorials/tenderly/debugger.png)
</Steps>