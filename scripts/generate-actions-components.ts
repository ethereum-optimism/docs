...
function generateComponentMDX(className: string, classDescription: string, methods: MethodDoc[]): string {
  let mdx = `{/*
...
async function main() {
  ...
  for (const [className, sourcePath] of Object.entries(ACTIONS_COMPONENTS)) {
    ...
    const componentContent = generateComponentMDX(className, classDescription, methods);
