...
function generateComponentMDX(className: string, classDescription: string, properties: PropertyDoc[], methods: MethodDoc[]): string {
  let mdx = `{/*
...
async function main() {
  ...
  for (const [className, sourcePath] of Object.entries(ACTIONS_COMPONENTS)) {
    ...
    const componentContent = generateComponentMDX(className, classDescription, properties, methods);
